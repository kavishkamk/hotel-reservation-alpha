...
const DashboardPage = () => {
	...
	useEffect(()=> {
		async function getUserByEmail(email){
			const data = await Dashboard__connection.getUserByEmail(email)
			console.log(data)
	...
	const clientRegisterHandler = async (client)=> {
		...
		const data = await Dashboard__connection.clientRegister(newClient)

		if(data.error) {
			setMessage_func(false, data.error);
			setMessageStatus_func();
		}else if(data.user) {
			setMessage_func(true, "New client successfully registered");
			setMessageStatus_func();
		}
		console.log(data)
