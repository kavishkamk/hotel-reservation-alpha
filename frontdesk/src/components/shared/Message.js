...
const Message = () => {
	...
	return (
		<>
			<div
				className={
					"fixed inset-x-0 inset-y-0 w-1/2 h-[15%] z-30 m-auto flex flex-row items-center p-6 border-2 border-l-8 bg-slate-100 text-gray-900 " +
					(message.status === true
						? "border-green-400"
						: "border-red-400")
				}
			...
			<div onClick={notifCloseHandler} className="fixed left-0 right-0 backdrop-blur-sm z-20 top-0 bottom-0"></div>
		</>
	);
};
